  ...
  @if(Session::has('success'))
    ...
    <div class="box-header with-border widget-user-2  bg-aqua widget-user-header padfull">
      ...
      <div class="col-md-8 col-sm-4 padzero">
        ...
        <h6 class="widget-user-desc">{{$client->email}}<br>@if($client->mobile_code)
            <b>+</b>{{$client->mobile_code}}@endif{{$client->mobile}}</h6>
      </div>
          ...
          <ul class="dropdown-menu">
            <li role="presentation">
              <a role="menuitem" tabindex="-1"
                 href="{{url('invoice/generate?clientid='.$client->id)}}">{{Lang::get('message.invoice')}}</a>
            </li>
            <!--                <li role="presentation">
                          ...
                          <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" role="menu">
                              @if($invoice->order()->get()->count()==0)
                                <li><a href=# class=null data-toggle='modal' data-target="#editinvoice{{$invoice->id}}">Execute
                                    order</a></li>
                              @endif

                              @if($invoice->status!='success')
                                <li><a
                                    href="{{url('payment/receive?invoiceid='.$invoice->id)}}">{{Lang::get('message.payment')}}</a>
                                </li>
                              @endif

                              <li><a
                                  href="{{url('invoices/show?invoiceid='.$invoice->id)}}">View {{Lang::get('message.invoice')}}</a>
                              </li>
                              <li><a
                                  href="{{url('invoices/'.$invoice->id.'/delete')}}">{{Lang::get('message.delete')}}</a>
                              </li>
                            </ul>
                            {!! $model_popup !!}
                ...
                <table id="example4" class="table table-bordered table-hover">
                  ...
                  @forelse($client->payment()->orderBy('created_at','desc')->get() as $payment)
                    <tr>
                      ...
                      <td><a href="{{url('payments/'.$payment->id.'/delete')}}"
                             class="btn btn-danger">{{Lang::get('message.delete')}}</a></td>
                    </tr>
                  @empty
                    <tr>
                      <td>
                ...
                <div>
                  ...
                  <div class="box-footer no-padding">
                    <ul class="nav nav-stacked">
                      ...
                      <li>
                        <a href="#">
                          <strong>{{Lang::get('message.mobile')}} :</strong> <span class="pull-right">@if($client->mobile_code)
                              <b>+</b>{{$client->mobile_code}}@endif{{$client->mobile}}</span>
                        </a>
                      </li>
                      <li>
                        <a href="#">
                          <strong>{{Lang::get('message.address')}} :</strong> <span
                            class="pull-right">{{ucfirst($client->address)}}</span>
                        </a>
                      </li>
                      <li>
                        <a href="#">
                          <strong>{{Lang::get('message.town')}} :</strong> <span
                            class="pull-right">{{ucfirst($client->town)}}</span>
                        </a>
                      </li>
                      <li>
                        <a href="#">
                      ...
                      <li>
                        <a href="#">
                          <strong>{{Lang::get('message.zip')}} :</strong> <span
                            class="pull-right">{{$client->zip}}</span>
                        </a>
                      </li>
                      <li>
                        <a href="#">
                          <strong>{{Lang::get('message.role')}} :</strong> <span
                            class="pull-right">{{ucfirst($client->role)}}</span>
                        </a>
                      </li>
                      <li>
                        <a href="#">
                          <strong>{{Lang::get('message.currency')}} :</strong> <span
                            class="pull-right">{{$client->currency}}</span>
                        </a>
                      </li>
                      <li>
                        <a href="#">
                          <strong>Company Type :</strong> <span
                            class="pull-right">{{str_replace('-',' ',ucfirst($client->company_type))}}</span>
                        </a>
                      </li>
                      <li>
                        <a href="#">
                          <strong>Company Size :</strong> <span
                            class="pull-right">{{str_replace('-',' ',ucfirst($client->company_size))}}</span>
                        </a>
                      </li>
                      <li>
                        <a href="#">
                      ...
                      @if($client && $manager)
                        <li>
                          <a href="{{url('clients/'.$manager->id)}}">
                            <strong>Account Manager :</strong> <span
                              class="pull-right">{{$manager->first_name}} {{$manager->last_name}}</span>
                          </a>
                        </li>
                      @endif
                    </ul>
